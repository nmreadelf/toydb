...
#include "server/raftserver_impl.h"
#include <string>
...
public:
  ToyDbServerImpl(const std::string id) : id_(id), raft_(){};

  ::grpc::Status Status(ServerContext *ctx, const StatusRequest *req,
                        StatusResponse *resp);

  ::grpc::Status Get(ServerContext *ctx, const GetRequest *req,
                     GetRequest *resp);

  ::grpc::Status Set(ServerContext *ctx, const SetRequest *req,
                     SetResponse *resp);

private:
  std::string id_;
  RaftServerImpl raft_;
};
} // namespace toydb
